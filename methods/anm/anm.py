...
class ANM(HerdingIndicesLearner):
    def __init__(self, data_manager, configs: dict, device, distributed=False) -> None:
        super().__init__(data_manager, configs, device, distributed)
        self._init_network(self.configs.get('backbone_configs', {}), self.configs.get('network_configs', {}))
    ...
    def _init_network(self, backbone_configs, network_configs):
        if backbone_configs.get('name') == 'resnet18':
            params = {
        ...
        self.network = ANMNet(backbone_configs, network_configs, self.device)
    ...
    def _model_to_ddp(self):
        self.network = nn.parallel.DistributedDataParallel(
            self.local_network, device_ids=[self.distributed['rank']],
            find_unused_parameters=self.configs.get('debug', False)
        )
    ...
    def extract_herding_features(self, dataset: torch.utils.data.Dataset) -> torch.Tensor:
        ...
        if self.configs.get('ffcv'):
            ...
            pipeline_name = self.configs.get('test_transform', default_transform_dict[dataset_name.strip('0123456789')][0])
            data_loader = get_ffcv_loader(
                dataset, self.configs['train_beton_path'], pipeline_name, self.device,
                order=OrderOption.SEQUENTIAL, seed=self.configs['seed'],
        ...
        features = features / (features.norm(dim=-1, keepdim=True) + EPSILON)
        return features
    ...
    def train(self) -> None:
        ...
        for task_id, (task_train, task_test) in enumerate(self.data_manager.tasks):
            ...
            self.update_state(cur_task=task_id+1, cur_task_num_classes=cur_task_num_classes,
                              sofar_num_classes=sofar_num_classes, learned_num_classes=learned_num_classes)


            self.local_network.update_network(cur_task_num_classes)
            self.local_network.freeze_old_backbones()
            if self.configs.get('ckpt_path') is not None and self.configs.get('ckpt_task') is not None and task_id + 1 <= self.configs['ckpt_task']:
            ...
            if self.configs.get('ffcv'):
                ...
                train_loader, test_loader = get_ffcv_loaders(
                    task_train, task_test, self.configs['trainloader_params'],
                    self.configs['testloader_params'], self.device, self.configs, self.distributed is not None
                )
            else:
                train_loader, test_loader = get_loaders(
                    task_train, task_test, self.configs['trainloader_params'],
                    self.configs['testloader_params'], self.distributed
                )
            ...
            self.print_logger.info(f"Adjust memory to {self.num_exemplars_per_class} per class "
                                   f"({self.num_exemplars_per_class * sofar_num_classes} in total).")
            self.reduce_memory()
            self.update_memory()
            ...
            if self.configs.get('ckpt_dir') is not None and not self.configs.get('disable_save_ckpt') and (task_id + 1) in self.configs.get('save_ckpt_tasks', []):
    ...
    def train_task(self, train_loader, test_loader):
        if self.state['cur_task'] == 1:
            ...
            num_epochs = self.configs['init_epochs']
        else:
            ...
            num_epochs = self.configs['inc_epochs']
        ...
        for epoch in prog_bar:
            ...
            if self.distributed and not self.configs.get('ffcv'):
            ...
            if epoch % self.configs.get('eval_interval', 1) == 0:
    ...
    def train_epoch(self, train_loader, optimizer, scheduler):
        ...
        if self.state['cur_task'] > 1:
            metrics.update({
                ...
                'distill_loss': MeanMetric().to(self.device)
            })
        ...
        for batch, batch_data in enumerate(train_loader):
            ...
            out = self.network(samples.contiguous(), task_id=str(cur_task))
            ...
            if (aux_logits := out.get('aux_logits')) is not None:
                ...
                ts_features = out['ts_features']

                distill_loss = l1_loss(ta_feature, ts_features)
                                              
                loss = cls_loss + self.configs['aux_factor'] * aux_loss + self.configs['distill_factor'] * distill_loss
            ...
            if self.state['cur_task'] > 1 and 'aux_loss' in locals():
                metrics['aux_loss'].update(aux_loss.detach())
                metrics['distill_loss'].update(distill_loss.detach())
...
def l1_loss(ta_feature, ts_features):
    return F.l1_loss(ts_features[-1], ta_feature)